...
var passwordCheckOld = function () //set up password check function
{
  var stringPassword, arrayBadPasswords, stringOutput; //declare variables
    
    stringPassword = $("password").value; //take password input from DOM and assign to stringPassword
    ...
    //lists 500 bad passwords
  stringOutput = ""; //sets stringOutput to be blank initially
 
   if (stringPassword.length <8) //checks to make sure password length greater than 8
    {
      stringOutput= "I'm sorry, the password " + stringPassword + " is less than 8 characters long.  Please try again."; //provides error message if password less than 8 characters
    }
    else
    {
      for (var i=0; i<arrayBadPasswords.length; i++) //sets up look to check passwords that meet minimum character requirement
      {
          if (arrayBadPasswords[i] === stringPassword) //checks stringPassword against each element in arrayBadPasswords
          {
           stringOutput = "I'm sorry, the password " + stringPassword + " is on the bad password list.  Please try again."; //if password matches a bad password, output error message
           break;
          }
      }
    }
    if (stringOutput != "") //if stringOuput isn't blank output it
    {
      $("output").value = stringOutput;
    }
    else //otherwise output the bad password and an error message
    {
      $("output").value = stringPassword + " is not on the bad password list!  Good job! Go ahead and use it!";
    }
};
...
{
//    $("password").value = "";
   $("password").focus(); //focuses on password box
   $("check_password").onclick = passwordCheckOld; //runs password check function when clicked
};